<template>
  <div>
    <v-toolbar flat class="elevation-2">
      ...
      <v-spacer></v-spacer>
      <v-btn color="cctBlue" outlined @click="loadStatistics" :loading="loading">
    ...
    <div class="px-1">
      <div v-if="loading" class="text-center py-12">
        <v-progress-circular indeterminate color="cctBlue" size="64"></v-progress-circular>
        <div class="mt-4 text-body-1">Statistiken werden geladen und aktualisiert…</div>
        <div class="text-caption cctGrey--text">Dies kann einige Sekunden dauern.</div>
      </div>
        ...
        <v-row class="mb-4 mt-4">
          <v-col cols="12" md="4">
            <v-card flat class="summary-card summary-card--active">
              <v-card-text class="text-center py-6">
                <div class="text-h3 font-weight-bold cctGreen--text">{{ activeCount }}</div>
                <div class="text-subtitle-1 cctGrey--text">Aktiv (alle Kriterien erfüllt)</div>
              </v-card-text>
            </v-card>
          </v-col>
          <v-col cols="12" md="4">
            <v-card flat class="summary-card summary-card--quote" :style="quoteCardStyle">
              <v-card-text class="text-center py-6">
                <div class="text-h3 font-weight-bold cctBlue--text">{{ activityPercentage }}%</div>
          ...
          <v-col cols="12" md="4">
            <v-card flat class="summary-card summary-card--passive">
              <v-card-text class="text-center py-6">
                <div class="text-h3 font-weight-bold cctOrange--text">{{ inactiveCount }}</div>
                <div class="text-subtitle-1 cctGrey--text">Passiv (Kriterien nicht erfüllt)</div>
              </v-card-text>
            </v-card>
          </v-col>
        </v-row>
        ...
        >
            <template v-slot:item.status="{ item }">
              <v-chip
                ...
                small
              >
                <v-icon left small>{{ item.is_activity_fulfilled ? 'mdi-check-circle' : 'mdi-alert-circle' }}</v-icon>
            ...
            <template v-slot:item.full_name="{ item }">
              <div class="d-flex align-center py-2">
                ...
                <div>
                  ...
                  <div class="text-caption cctLightGrey--text">{{ item.ressort }}</div>
                </div>
              </div>
            </template>

            <template v-slot:item.memberstatus="{ item }">
              <v-chip small outlined color="cctBlue">{{ item.memberstatus || '–' }}</v-chip>
            </template>

            <template v-slot:item.dosi_attendance_quota="{ item }">
              <div class="d-flex align-center">
                <v-icon :color="item.dosi_ok ? 'cctGreen' : 'error'" small class="mr-1">
            ...
            <template v-slot:item.ressort_attendance_quota="{ item }">
              <div class="d-flex align-center">
                <v-icon :color="item.ressort_ok ? 'cctGreen' : 'error'" small class="mr-1">
            ...
            <template v-slot:item.applications_or_staffing="{ item }">
              <div class="d-flex align-center">
                <v-icon :color="item.applications_or_staffing_ok ? 'cctGreen' : 'error'" small class="mr-1">
                  {{ item.applications_or_staffing_ok ? 'mdi-check' : 'mdi-close' }}
                </v-icon>
                <span :class="item.applications_or_staffing_ok ? 'cctGreen--text' : 'error--text'">
                  {{ item.project_applications_count || 0 }} / {{ item.project_staffing_count || 0 }}
                </span>
            ...
            <template v-slot:item.workshop_participation_count="{ item }">
              <div class="d-flex align-center">
                <v-icon :color="item.workshop_ok ? 'cctGreen' : 'error'" small class="mr-1">
                  {{ item.workshop_ok ? 'mdi-check' : 'mdi-close' }}
                </v-icon>
                <span :class="item.workshop_ok ? 'cctGreen--text' : 'error--text'">
            ...
            <template v-slot:item.actions="{ item }">
              <v-btn icon small :to="{ name: 'profile', params: { id: item.id } }">
...
export default class StatsDashboard extends Vue {
  ...
  headers = [
    ...
    { text: 'Memberstatus', value: 'memberstatus', sortable: true, class: 'text-no-wrap' },
    { text: 'DoSi Quote', value: 'dosi_attendance_quota', sortable: true, class: 'text-no-wrap' },
    { text: 'Ressort Quote', value: 'ressort_attendance_quota', sortable: true, class: 'text-no-wrap' },
    { text: 'Bewerbungen / Besetzungen', value: 'applications_or_staffing', sortable: true, class: 'text-no-wrap' },
    { text: 'Workshops', value: 'workshop_participation_count', sortable: true, class: 'text-no-wrap' },
  ...
  get memberstatusOptions() {
    ...
    return [{ text: 'Alle', value: null }, ...statuses.map(s => ({ text: s, value: s }))];
  }
  ...
  get quoteCardStyle() {
    const percentage = this.users.length > 0 ? (this.activeCount / this.users.length) * 100 : 0;
    return {
  ...
  async loadStatistics() {
    ...
    try {
      ...
      if (err.response?.status === 403) {
        this.error = 'Keine Berechtigung. Diese Ansicht ist nur für den Vorstand verfügbar.';
      } else {
